function[STR,HZM]=STRMAT(D,X,Y,T,DS)
STR=0*(1:6);
...
B=B/(2*A);
HZM=B*DS';STR(1:3)=D*HZM;
SX=STR(1);SY=STR(2);TXY=STR(3);
SQ=sqrt((SX-SY)^2+4*TXY^2);
STR(4)=0.5*(SX+SY+SQ);STR(5)=0.5*(SX+SY-SQ);
STR(6)=0.5*atan(2*TXY/(SX-SY));
